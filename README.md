...
Digits Only: Generates an OTP composed exclusively of numerical digits (0-9).

Uppercase Alphanumeric: Generates an OTP composed of uppercase letters (A-Z) and digits (0-9).

Lowercase Alphanumeric: Generates an OTP composed of lowercase letters (a-z) and digits (0-9).

All Characters: Generates an OTP using a mix of lowercase letters, uppercase letters, and digits.

Customizable Length: The length of the generated OTP is defined when initializing the SimpleOTP class.
...
Verify Length: Check that all four output strings have the length specified during initialization (7 in the provided example).

Verify Randomness: Run the script multiple times (at least 5-10 times). The output OTPs for each property should be unique every time.

Verify Character Sets (Per Property):
...
<img width="303" height="93" alt="Screenshot 2025-11-25 021135" src="https://github.com/user-attachments/assets/a1d0924f-1d78-4251-9e2b-4fae8fab72b3" />
