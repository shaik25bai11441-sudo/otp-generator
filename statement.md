...
Configurable Length: The core function is to generate an OTP of a user-defined length, set when the SimpleOTP object is created.

Digit-Only Generation: A specific feature (otp_generator.digits) generates codes composed exclusively of numbers (0 through 9).

Uppercase Alphanumeric: A feature (otp_generator.uppercase_digits) generates codes using a combination of uppercase letters and numerical digits.

Lowercase Alphanumeric: A feature (otp_generator.smallcase_digits) generates codes using a combination of lowercase letters and numerical digits.

Full Alphanumeric: The most comprehensive feature (otp_generator.all) generates codes using all available characters: lowercase letters, uppercase letters, and numerical digits.

Underlying Flexibility: The core generate_otp(char\_set) method ensures the class can be easily extended to use any custom set of characters.
